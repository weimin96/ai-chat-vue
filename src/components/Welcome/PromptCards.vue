...
<template>
  <div :style="`display:grid;gap:8px;width:100%;max-width:480px;grid-template-columns:repeat(${columns ?? 2},1fr)`">
    <button
      ...
      class="ac-prompt-card group p-3 bg-white border border-[var(--ac-border,#e5e7eb)] rounded-xl text-left hover:border-[var(--ac-primary,#6366f1)] hover:shadow-sm transition-all"
    >
      <span v-if="card.icon" class="text-base block mb-1">{{ card.icon }}</span>
      <p class="text-xs font-medium text-[var(--ac-text,#1f2937)] group-hover:text-[var(--ac-primary,#6366f1)] transition-colors">{{ card.title }}</p>
